...
1. queryfadir (-qf): Path to the fasta file of query seq (e.g. functional regions of a ncRNA). If query fasta contains more than one sequence, all the sequences in query fasta file will be merged to one sequence for calculating seekrPearson and for calculating kmer count files for hmseekr
...
2. queryfadir (-qf): Path to the fasta file of query seq (e.g. functional regions of a ncRNA). If query fasta contains more than one sequence, all the sequences in query fasta file will be merged to one sequence for calculating seekrPearson and for calculating kmer count files for hmseekr
...
#### Python Example:
search all genes on chr16 for the potential hit counts and similarities to the query sequences include mXist repeat A, B, C and E, filtering and keep hit sqeuences with length filter provided in lenfilter, and p val less than 0.5 for stats calculation. the conditioned emission findhits_condE function is used, print the output in both long and wide format
...
testsum = seqstosummary(queryfadir='./fastaFiles/mXist_repeats.fa', 
                        ...
                        func='findhits_condE',pfilter=0.5,
...
9. pfilter (-pf): only keep hits sequences that have seekr pearson correlation p value < pfilter for calculating stats in the output, default=1.1. if no filter is needed, set to 1.1
10. outputname (-name): File name for output dataframe, default='seqstosummary_results'
11. outputdir (-dir): path of output directory to save outputs and intermediate files, default is a subfolder called seqstosummary under current directory. The intermediate fasta seq files, count files, trained models and hits files are automatically saved under the outputdir into subfolders: seqs, counts, models, hits, where qT and nT are included in the file names
12. outdfformat (-format): the format of the output dataframe, default='both' where both long and wide format would be saved, other options are 'wide' or 'long'
...
If you want to work with python codes instead of directly calling from the command line, you can choose to run hmseekr with Jupyter Notebook inside the Docker Container.
